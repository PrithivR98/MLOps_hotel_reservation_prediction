...
class DataProcesssor:
    ...
    def preprocess_data(self,df):
        try:
            ...
            logger.info("Dropping unneccesary columns")
            ...
            logger.info("HANDLING THE SKEWNESS IN DATA")
            
            skew_threhold = self.config["data_processing"]['skewness_threshold']
            skewness = df[num_cols].apply(lambda x:x.skew())
            
            for column in skewness[skewness>skew_threhold].index:
                df[column] = np.log1p(df[column])
                
            return df
        except Exception as e:
            logger.error(f"Error duting preprocess step {e}")
            raise CustomException("Error while preprocess data",e)
        
    def balance_data(self,df):
        try:
            logger.info("Handling imbalaced data")
            ...
            logger.error(f"Error duting balancing data step {e}")
            raise CustomException("Error while balancing data step",e)
    ...
    def select_features(self,df):
        try:
            ...
            logger.error(f"Error during feature selection step {e}", exc_info=True)
...
if __name__=="__main__":
    processor = DataProcesssor(TRAIN_FILE_PATH,TEST_FILE_PATH,PROCESSED_DIR,CONFIG_PATH)
    processor.process()       
